        ...
        private void WriteStmt(TextWriter output, Function function, IPStmt stmt)
        {
            ...
            switch (stmt)
            {
                ...
                case AssignStmt assignStmt:
                    ...
                    if (assignStmt.Value is CtorExpr) {
                        ...
                        if (argVariable == null) {
                            ...
                            string this_ref_temp = context.Names.GetTemporaryName("this_ref");
                            context.Write(output, $"PRT_VALUE* {this_ref_temp} = ");
                            WriteExpr(output, function, argVariable);
                            context.WriteLine(output, ";");
                            context.WriteLine(output, $"_P_GEN_funargs[{i}] = &{this_ref_temp};");
                ...
                case UntrustedSendStmt untrustedSendStmt:
                    {
                        string intLiteralName = context.RegisterLiteral(function, untrustedSendStmt.Arguments.Count);
                        string uniqueTempVariable = context.Names.GetTemporaryName("PTMP_tmp"); //TODO find a cleaner way to do this
                        ...
                        context.Write(output, "_P_GEN_funargs[0] = ");
                        IVariableRef mExpr = (IVariableRef)untrustedSendStmt.MachineExpr;
                        context.Write(output, $"{GetVariableReference(function, mExpr)}");
                        context.WriteLine(output, ";");

                        context.Write(output, "_P_GEN_funargs[1] = &(");
                        WriteExpr(output, function, untrustedSendStmt.Evt);
                        context.WriteLine(output, ");");
                        ...
                        foreach (IPExpr sendArgExpr in untrustedSendStmt.Arguments)
                        {
                            Debug.Assert(sendArgExpr is IVariableRef);
                            IVariableRef argVar = (IVariableRef)sendArgExpr;
                            context.WriteLine(output, $"_P_GEN_funargs[{i_in_loop}] = {GetVariableReference(function, argVar)};");
            ...
            switch (expr)
            {
                ...
                case CtorExpr ctorExpr:
                    ...
                    if (ctorExpr.Interface.IsSecure) {
                        ...
                        context.Write(
                            output,
                            $"((_P_GEN_funval = P_CreateUSMMachineRequest_IMPL(context, _P_GEN_funargs)), (_P_GEN_funval))");
